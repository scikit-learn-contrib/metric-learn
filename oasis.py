...
class _BaseOASIS():
    ...
    c:      passive-agressive param. Controls trade-off bewteen
            remaining close to previous W_i-1 OR minimizing loss of
            current triplet.
    ...
    def _fit(self, triplets):
        ...
        # W_0 = I, Here and once, to reuse self.components_ for partial_fit
    ...
    def _score_pairs(self, pairs):
        ...
        """
        return np.diagonal(
            np.dot(np.dot(pairs[:, 0, :], self.components_), pairs[:, 1, :].T))
    ...
    def _loss(self, triplet):
        ...
        return np.maximum(0, 1 -
                          self._score_pairs([[triplet[0], triplet[1]], ])[0] +
                          self._score_pairs([[triplet[0], triplet[2]], ])[0])
