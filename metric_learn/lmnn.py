...
class _BaseLMNN(MahalanobisMixin, TransformerMixin, _TripletsClassifierMixin):
  """Large Margin Nearest Neighbor (LMNN)"""

  _tuple_size = 3   # constraints are triplets

  def __init__(self, init='auto', n_neighbors='infer', min_iter=50, max_iter=1000,
  ...
  def _fit(self, triplets):
    ...
    if not isinstance(self.min_iter, int):
      raise ValueError("max_iter should be an integer, instead it is of type"
                       " %s" % type(self.min_iter))

    if(self.min_iter > self.max_iter):
      raise ValueError("The value of min_iter must be equal or smaller than"
                       " max_iter.")
    ...
    if self.n_neighbors == 'infer':
        self.n_neighbors = k = self._infer_n_neighbors(triplets)
    else:
        k = self.n_neighbors
    ...
    # in the future.
    triplets, X, label_mask = self._to_index_points(triplets)
    num_pts, d = X.shape

    output_dim = _check_n_components(d, self.n_components)

    self.components_ = _initialize_components(output_dim, X, None, self.init,
                                              self.verbose,
                                              random_state=self.random_state,
                                              has_classes=False)


    target_neighbors = self._select_targets(X, label_mask)
  ...
  def _loss_grad(self, X, L, dfG, k, reg, target_neighbors, label_mask):
  ...
  def _to_index_points(self, triplets):
    ...
    ## pairwise label mask implied by the triplets
    label_mask = np.zeros((X.shape[0], X.shape[0]))
    for (i, j, k) in triplets:
        label_mask[i,j] = 1
        label_mask[i,k] = -1
    return triplets, X, label_mask

  def _select_targets(self, X, label_mask):
    dd = euclidean_distances(X, squared=True)
    np.fill_diagonal(dd, np.inf)
    dd[label_mask==-1] = np.inf
    return np.argsort(dd)[..., :self.n_neighbors]

  def _find_impostors(self, furthest_neighbors, X, label_mask, L):
    Lx = X.dot(L.T)
    margin_radii = 1 + _inplace_paired_L2(Lx[furthest_neighbors], Lx)
    impostors = []
    # Find imposters from label mask
    (i, j) = (label_mask==-1).nonzero()
    dist = np.square(paired_euclidean_distances(Lx[i], Lx[j]))
    imposters = dist < margin_radii
    return np.array([i[imposters],j[imposters]])

  def _infer_n_neighbors(self, triplets):
    # from the triplet index set, infer the number of neighbors via the number
    # of unique target/imposters + target/genuine pairs
    k_genuine = np.unique(triplets[:,[0,2]], axis=0, return_counts=True)[1].min()
    k_imposter = np.unique(triplets[:,[0,1]], axis=0, return_counts=True)[1].min()
    return k_genuine + k_imposter
...
class LMNN(_BaseLMNN):
  ...
  The learned metric attempts to keep close k-nearest neighbors, while keeping
  potential imposters separated by a large margin. This algorithm makes no
  ...
  n_neighbors : int, optional (default='infer')
    Number of neighbors to consider, not including self-edges. If 'infer',
    the number of nearest neighbors can be inferred by counting point pairs
    from amoung the provided triplets.
...
class LMNN_Supervised(MahalanobisMixin, TransformerMixin):
  """Large Margin Nearest Neighbor (LMNN)
  ...
  def __init__(self, init='auto', n_neighbors=3, min_iter=50, max_iter=1000,
    ...
    self.n_components = n_components
    self.random_state = random_state
    super(LMNN_Supervised, self).__init__(preprocessor)
